        ...
        for (序号, 一字信息项) in zip(&self.一字索引, &self.一字信息) {
            ...
            *序列 = [0; 4];
            ...
            if 方案 == 字源方案::前缀 {
                ...
                let 全拼顺取 = 一字信息项.全拼顺取;
                if 序列[1] == 0 {
                    // 1个字根的字，根据拼音长度决定辅助码数量
                    // 如果全拼只有一个字符，则辅助码只取一码
                    if 全拼顺取[0] == 全拼顺取[1] && 全拼顺取[0] == 全拼顺取[2] {
                        // 拼音只有1个字符，只取这1个字符作为辅助码
                        (序列[1], 序列[2], 序列[3]) = (全拼顺取[0], 0, 0);
                    } else {
                        // 拼音有2个及以上字符，取第一个字符和最后一个字符作为辅助码
                        (序列[1], 序列[2], 序列[3]) = (全拼顺取[0], 全拼顺取[2], 0);
                    }
                } else if 序列[2] == 0 {
                    序列[2] = 全拼顺取[0];
                } else if 序列[3] == 0 {
