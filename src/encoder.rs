...
use chai::{encoders::编码器, 元素, 元素映射, 棱镜, 编码信息, 部分编码信息, 错误};
...
impl 字源编码器 {
    ...
    pub fn 构建元素序列(&mut self, 映射: &Vec<u64>, 决策: &字源决策) {
        ...
        for (指针, (_块序号, 拆分方式列表)) in zip(&mut 当前拆分索引, self.动态拆分.iter().enumerate()) {
            let mut 找到 = false;
    ...
    fn 输出简码(&mut self) {
        for (_序号, 编码信息) in self.编码结果.iter_mut().enumerate() {
            if 编码信息.词长 > 1 {
                continue;
            }
            let 全码 = 编码信息.全码.原始编码;
